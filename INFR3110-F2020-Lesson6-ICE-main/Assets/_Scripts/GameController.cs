...
public class GameController : MonoBehaviour
{
	...
	public TileManagerBehavior tile_manager;
    ...
    void Start()
    {
		tile_manager = FindObjectOfType<TileManagerBehavior>();
    ...
    public bool AddTile(Vector3 new_location)
    {
        ...
        {
			if (Tiles.Count <= tile_manager.MAX_TILES - 1)
			{
				var newTile = tile_manager.GetTile();
    ...
    IEnumerator ThresholdCheck()
    {
		...
		if ((float)Tiles.Count < ((float)tile_manager.MAX_TILES * Threshold))
		{
			_Reset();
		}
    ...
    private void _Reset()
    {
		if (Tiles.Count > 0)
		{
			...
			{
				tile_manager.ReturnTile(child.gameObject);
			}
		}
